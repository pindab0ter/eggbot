...
object Transport {
    fun multiplierFor(backup: Backup, farm: Backup.Farm): BigDecimal = farm.baseShippingRate
        .multiply(farm.shippingRateCommonResearchMultipliers
            .plus(backup.shippingRateEpicResearchMultiplier)
            .product()
            .multiply(backup.activeSoloArtifactsFor(farm).shippingRateMultiplier)
        )
    ...
    private val List<Artifact>.shippingRateMultiplier
        get() = productOf { artifact ->
            Artifacts.multiplierFor(artifact, shippingRateArtifacts)
        }
